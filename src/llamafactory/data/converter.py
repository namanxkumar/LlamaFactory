        ...
        if self.dataset_attr.load_from in ["script", "file"]:
            if isinstance(medias[0], str):
                for i in range(len(medias)):
                    ...
                    # Always use the absolute media_path even when the loose file is absent
                    # (it may live inside a sibling tar shard). _resolve_tar_image in
                    # mm_plugin.py uses dirname(path) to locate tars — it needs an absolute
                    # path pointing at the correct directory, not the original relative path.
                    medias[i] = media_path
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        for turn_idx, message in enumerate(messages):
            ...
            if "loss_weight" in message:
                aligned_msg["loss_weight"] = float(message["loss_weight"])
            aligned_messages.append(aligned_msg)
